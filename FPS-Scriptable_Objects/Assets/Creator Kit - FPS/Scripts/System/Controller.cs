...
public class Controller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < playerWeaponsObj.startingWeapons.Length; ++i)
        {
            PickupWeapon(playerWeaponsObj.startingWeapons[i]);
        }

        for (int i = 0; i < playerWeaponsObj.startingAmmo.Length; ++i)
        {
            ChangeAmmo(playerWeaponsObj.startingAmmo[i].ammoType, playerWeaponsObj.startingAmmo[i].amount);
        }
        ...
        for (int i = 0; i < playerWeaponsObj.startingAmmo.Length; ++i)
        {
            m_AmmoInventory[playerWeaponsObj.startingAmmo[i].ammoType] = playerWeaponsObj.startingAmmo[i].amount;
        }
    ...
    void Update()
    {
        ...
        if (!m_IsPaused && !LockControl)
        {
            ...
            float actualSpeed = running ? PlayersObj.runningSpeed : PlayersObj.playerSpeed;
            ...
            // Turn player
            float turnPlayer =  Input.GetAxis("Mouse X") * PlayersObj.mouseSensitivity;
            ...
            turnCam = turnCam * PlayersObj.mouseSensitivity;
            ...
            Speed = move.magnitude / (PlayersObj.playerSpeed * Time.deltaTime);
        ...
        {
            FootstepPlayer.PlayClip(PlayersObj.landingAudioClip, 0.8f,1.1f);
        }
    }
    ...
    void PickupWeapon(Weapon prefab)
    {
        ...
        {//if we already have that weapon, grant a clip size of the ammo type instead
            ChangeAmmo(prefab.weaponsObj.ammoType, prefab.weaponsObj.clipSize);
        }
        else
        {
    ...
    public void ChangeAmmo(int ammoType, int amount)
    {
        ...
        if (m_Weapons[m_CurrentWeapon].weaponsObj.ammoType == ammoType)
        {
